...
export default function ViewProfile({userid,dropdown,setDropdown, isAuthenticated})
{
  let {id}=useParams();
  console.log(id)
  ...
  useEffect(()=>{
    ...
    return(
         <div onClick={handleDropdown} className=' h-screen bg-white'>
...
<div className='mx-10  md:mx-4 flex h-10'><FcDocument size={30}/><span className='mx-2 font-pat font-bold  text-lg'>{name}'s notes</span></div>


</div>
<div>
 {data.length>0 ?<div className='grid grid-cols-3 md:grid-cols-1 mt-10  mr-20 md:mr-0  text-main  '>
{data.map((item)=>{
